...
/**
* Plugin Name:       Upload Stripe Payments
* Description:       Upload and Easily accept credit card payments via Stripe payment gateway in WordPress.
* Version:           1.0.0
* Author:            vmdao
* Author URI:        https://vuminhdao.info/
* Plugin URI:        https://vuminhdao.info/
...
add_action('wp_ajax_custom_upload', 'usp_checkForPublicSubmission');
add_action('wp_ajax_nopriv_custom_upload', 'usp_checkForPublicSubmission');
...
function usp_checkForPublicSubmission() {
	...
	if (isset($_POST['user-submitted-post'], $_POST['usp-nonce']) && !empty($_POST['user-submitted-post']) && wp_verify_nonce($_POST['usp-nonce'], 'usp-nonce')) {
		...
		if ($post_id) {
			$status = ['status' =>'200', 'message'=> 'Succed'];
		} else {
			$status = ['status' =>'400', 'message'=> 'Unsucced'];
		}
		echo json_encode($status );
		exit();
	}
